...
export class ComponentBase<
    ...
    > extends CoreComponent.Others.Abstract.Abstract_ComponentConnector{
    ...
    connectedCallback() {
        this._unsubscribeDirection =
            CoreObservable.Observable.computed(
                ( dir) => {
                    this.set("directionRtl", dir);
                    },
                [
                    CoreConfigs.AppConfig.get_directionRtl()
                ],
                this.getScope());
        ...
        if (config) {
            ...
            Object.keys(props).forEach(key => {
                ...
                if (itemProp && itemProp.hasOwnProperty("prop")) {
                    ...
                    if (CoreObservable.Observable.isObservable(value)){
                        this._COMPONENT_PROPS_BIND[propName] = value;
                    }
                    else {
                        this._COMPONENT_PROPS_BIND[propName] = new CoreObservable.Observable(value);
                    }
                }
            })
        }
            ...
            Object.keys(this._COMPONENT_SCHEMA).forEach(key=> {
                ...
                if (itemPart && itemPart.hasOwnProperty("part") && itemPart.part == partName){
                    if (itemPart.hasOwnProperty("props")){
                        ...
                        const attrsDefault : CoreComponent.Others.Types.Type_PartAttrDefault = {
